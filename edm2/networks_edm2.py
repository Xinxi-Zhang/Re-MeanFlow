...
def mp_sum_2(a, b, t=0.5):
    return (a + (b - a) * t) / torch.sqrt((1 - t) ** 2 + t ** 2)
...
class UNet(torch.nn.Module):
    def __init__(self,
        ...
        **block_kwargs,                     # Arguments for Block.
        ...
        self.emb_noise_t = MPConv(cnoise, cemb, kernel=[])
    ...
    #### Original EDM2 U-Net model forward.
    def forward_og(self, x, noise_labels, class_labels):
        # Embedding.
    ...
    #### New EDM2 U-Net model forward for Re-MeanFlow.
    def forward(self, x, sigma_t, sigma_r, class_labels):
        # Time Embedding, mix of t and r
        emb = mp_sum(self.emb_noise_t(self.emb_fourier(sigma_t)), self.emb_noise_r(self.emb_fourier(sigma_r)), t=0.5)
...
#### Original Preconditioning.
@persistence.persistent_class
class Precond_og(torch.nn.Module):
    def __init__(self,
        img_resolution,         # Image resolution.
        img_channels,           # Image channels.
        label_dim,              # Class label dimensionality. 0 = unconditional.
...
#### New Preconditioning for Re-MeanFlow.
@persistence.persistent_class
class Precond(torch.nn.Module):
    ...
    def forward(self, x_, t, r, y=None, force_fp32=False, return_logvar=False, **unet_kwargs):

        # Convert flow to diff
        sigma = t / (1-t)
        x = x_ / (1-t.view(-1, 1, 1, 1))
        ...
        # Run the model.
        x_in = (c_in * x).to(dtype)
        F_x = self.unet.forward(x_in, t, r, class_labels, **unet_kwargs)
        D_x = c_skip * x + c_out * F_x.to(torch.float32)

        v = (x_ - D_x) / t.view(-1, 1, 1, 1)
        return v
    
    def forward_og(self, x_, t, r, y=None, force_fp32=False, return_logvar=False, **unet_kwargs):
        # Convert flow to diff
        sigma = t / (1-t)
        x = x_ / (1-t.view(-1, 1, 1, 1))
        ...
        # Run the model.
        x_in = (c_in * x).to(dtype)
        F_x = self.unet.forward_og(x_in, c_noise, class_labels, **unet_kwargs)
        D_x = c_skip * x + c_out * F_x.to(torch.float32)
