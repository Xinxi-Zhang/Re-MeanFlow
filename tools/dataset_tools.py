...
#----------------------------------------------------------------------------
@cmdline.command()
@click.option('--dataset-root', help='Path to converted dataset (folder or zip)', metavar='PATH', type=str, required=True)
@click.option('--dest',         help='Output JSON file',                        metavar='PATH', type=str, required=True)
@click.option('--num-passes',   help='How many times to traverse the dataset (for imagenet mode)', metavar='INT', type=int, default=4, show_default=True)
@click.option('--mode',         type=click.Choice(['imagenet', 'random']), default='imagenet', show_default=True, help='imagenet: match dataset classes; random: random 5M from 1000 classes')
@click.option('--json-path',    help='Path to JSON file', metavar='PATH', type=str, required=False)
def make_reflow_index(
    dataset_root: str,
    dest: str,
    num_passes: int,
    mode: str,
    ...
    """Create an index JSON for reflow sampling with file and label lists."""
    out = {}
    if mode == "imagenet":
        if num_passes < 1:
            raise click.ClickException('--num-passes must be >= 1')

        # Load labels from dataset.json written by `convert`.
        if os.path.isdir(dataset_root):
            if json_path is None:
                meta_path = os.path.join(dataset_root, 'dataset.json')
            else:
                meta_path = json_path
            if not os.path.isfile(meta_path):
                raise click.ClickException(f'Could not find dataset.json under {dataset_root}')
            with open(meta_path, 'r') as f:
                metadata = json.load(f)

        labels_raw = metadata.get('labels', None)
        if labels_raw is None:
            raise click.ClickException('dataset.json does not contain labels; cannot match class distribution')

        # labels_raw is [["00000/img00000000.png", 6], ...]
        img_labels = [int(lbl) for _, lbl in labels_raw]

        # Repeat labels num_passes times
        repeated_labels = img_labels * num_passes
        num_entries = len(repeated_labels)

        file_list = [f'{i+1}.npy' for i in range(num_entries)]
        label_list = repeated_labels

        out = {
            'root': os.path.dirname(dest) + '/samples',
            'file_list': file_list,
            'label_list': label_list,
        }

    elif mode == "random":
        num_entries = 1_300_000 * num_passes
        num_classes = 1000
        rng = np.random.default_rng(seed=42)
        label_list = rng.integers(low=0, high=num_classes, size=num_entries).tolist()
        file_list = [f'{i+1}.npy' for i in range(num_entries)]
